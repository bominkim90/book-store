...
async function loginController(req, res) {
  try {
    ...
    // 그럼 먼저, id가 존재하는지 확인
    console.log("id : ",id)
    const [result_id, _] = await conn.query(`SELECT salt FROM users WHERE id = ?`, id)

    const {salt} = result_id[0] // 데이터는 여러개 일 수 있어서 => 배열json형태로 들어온다
    ...
    const [result, __] = await conn.query(
